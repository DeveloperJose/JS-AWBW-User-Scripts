...
var awbw_music_player = (function (exports) {
  ...
  function moveDivToOffset(div, dx, dy, steps, ...followUpAnimations) {
      if (steps <= 1) {
          if (!followUpAnimations || followUpAnimations.length === 0)
              return;
  ...
  function getCurrentGameDay() {
      ...
      if (typeof lastData === "undefined")
          return gameDay;
      if (typeof lastData.day === "undefined")
          return gameDay;
      return lastData.day;
  }
  /**
  ...
  function getCurrentThemeType() {
      ...
      if (currentPowerState === "Y")
          return SettingsThemeType.CO_POWER;
      if (currentPowerState === "S")
          return SettingsThemeType.SUPER_CO_POWER;
  ...
  class musicPlayerSettings {
      ...
      static set alternateThemeDay(val) {
          if (this.__alternateThemeDay === val)
              return;
  ...
  function onSettingsChange$2(_key, _isFirstLoad) {
      ...
      if (_key === "themeType" || _key === "currentRandomCO")
          return "";
  ...
  function getMusicURL(coName, gameType, themeType, useAlternateTheme) {
      ...
      if (themeType === null || themeType === undefined)
          themeType = musicPlayerSettings.themeType;
      ...
      if (coName === "victory")
          return VICTORY_THEME_URL;
      if (coName === "defeat")
          return DEFEAT_THEME_URL;
      // Override the game type to a higher game if the CO is not available in the current game.
      if (gameType !== SettingsGameType.DS && AW_DS_ONLY_COs.has(coName))
          gameType = SettingsGameType.DS;
      if (gameType === SettingsGameType.AW1 && AW2_ONLY_COs.has(coName))
          gameType = SettingsGameType.AW2;
  ...
  function getCurrentThemeURLs() {
      ...
      coNames.forEach((name) => {
          ...
          if (specialLoops.has(name))
              audioList.add(regularURL.replace(".ogg", "-loop.ogg"));
      });
      return audioList;
  }
  /**
  ...
  class CustomMenuSettingsUI {
      ...
      constructor(prefix, buttonImageURL, hoverText = "") {
          ...
          document.addEventListener("click", (event) => {
              ...
              if (elmnt.id.startsWith(prefix) || elmnt.id === "overDiv")
                  return;
       ...
       */
      setImage(imageURL) {
          let btnImg = this.groups.get("img");
          btnImg.src = imageURL;
      }
      /**
      ...
      addItemToTable(name, item) {
          ...
          if (currentItemsInRow >= maxItemsPerRow)
              table.insertRow();
  ...
  (function (Description) {
      ...
      Description["UI_Volume"] = "Adjust the volume of the UI sound effects like moving your cursor, opening menus, and selecting units.";
      Description["Alternate_Day"] = "After what day should alternate themes like the Re-Boot Camp factory themes start playing? Can you find all the hidden themes?";
      ...
      Description["DS"] = "Play the Advance Wars: Dual Strike soundtrack. A bit better quality than with the DS speakers though.";
  ...
  function playSFX(sfx) {
      ...
      if (!musicPlayerSettings.captureProgressSFX && sfx === GameSFX.unitCaptureProgress)
          return;
      if (!musicPlayerSettings.pipeSeamSFX && sfx === GameSFX.unitAttackPipeSeam)
          return;
  ...
  function preloadAudios(audioURLs, afterPreloadFunction = () => { }) {
      ...
      let onAudioPreload = (event) => {
          ...
          if (numLoadedAudios >= audioURLs.size) {
              numLoadedAudios = 0;
              if (afterPreloadFunction)
                  afterPreloadFunction();
          }
          if (event.type === "error") {
  ...
  function onCursorMove(cursorX, cursorY) {
      ...
      if (timeSinceLastCursorCall < CURSOR_THRESHOLD_MS)
          return;
      if (cursorMoved) {
  ...
  function onFire(response) {
      ...
      let madeSCOPAvailable = (!couldAttackerActivateSCOPBefore && canAttackerActivateSCOPAfter) ||
          (!couldDefenderActivateSCOPBefore && canDefenderActivateSCOPAfter);
      let madeCOPAvailable = (!couldAttackerActivateCOPBefore && canAttackerActivateCOPAfter) ||
  ...
  function wiggleTile(div, startDelay = 0) {
      ...
      let deltaY = 0.05;
      let wiggleAnimation = () => {
          moveDivToOffset(div, deltaX, 0, stepsX, { then: [0, -deltaY, stepsY] }, { then: [-deltaX * 2, 0, stepsX] }, { then: [deltaX * 2, 0, stepsX] }, { then: [0, -deltaY, stepsY] }, { then: [-deltaX * 2, 0, stepsX] }, { then: [deltaX * 2, 0, stepsX] }, { then: [0, deltaY, stepsY] }, { then: [-deltaX * 2, 0, stepsX] }, { then: [deltaX, 0, stepsX] }, { then: [0, deltaY, stepsY] });
      };
